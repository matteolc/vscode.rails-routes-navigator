...
export class Routes {
  ...
  private async parse(line: string): Promise<Route | undefined> {
    ...
    const routesFragments = line
      .match(/\s*([\s\w]+)\s+([|\w]+)\s+([-_/\w().:]+)\s+([/\w]+)#([/\w]+)/)
      ?.slice(1)
    if (routesFragments) {
      return new Route(
...
class Route {
  ...
  constructor(
    ...
    this.prefix =
      prefix === ' ' && previousRoute && controller === previousRoute.controller
        ? previousRoute.prefix
        : prefix
    this.verb = verb
    this.uri = uri
  ...
  public isMatchedRoute(keywords: Array<string>): boolean {
    const target = `${this.prefix}${this.verb.toLowerCase()}${this.verb}${
      this.uri
